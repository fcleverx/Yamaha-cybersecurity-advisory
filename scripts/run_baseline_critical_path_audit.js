...
async function run() {
  ...
  await check("config-source-add-delete-and-persistence", async (item) => {
    ...
    await page.fill("#feed-source-name", "Wave1 Source");
    await page.fill("#feed-source-url", "https://example.com/feed.xml");
    await page.locator("button:has-text('Add Source')").first().click();
    await page.waitForTimeout(350);
    const sourceListText = ((await page.locator("#feed-sources-dashboard").textContent()) || "");
    const sourceAdded = sourceListText.includes("Wave1 Source");
    item.notes.push(`custom source added: ${sourceAdded}`);
    if (sourceAdded) {
    ...
    if (!sourceAdded || loadedSoc !== markerSoc) {
      item.status = "fail";
      item.notes.push("Source add/delete or config persistence failed.");
    }
  });
