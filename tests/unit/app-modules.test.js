...
test("NewsletterBuilder catalog exposes the 23 accessible template definitions", () => {
  const context = createContext();
...
test("NewsletterBuilder catalog exposes 23 accessible template definitions", () => {
  const context = createContext();
  loadScript(context, "js/newsletter_builder.js");
  const catalog = context.App.NewsletterBuilder.getTemplateCatalog();
  assert.equal(catalog.length, 23);
