...
test.describe("pages, guards, and storage", () => {
  ...
  test("config SOC email and custom feed source persist locally", async ({ page }) => {
    await resetStorage(page, "/config.html");

    await page.locator("#cfg-soc").fill("wave-soc@example.test");
    await page.locator("#feed-source-name").fill("Wave QA Source");
    await page.locator("#feed-source-url").fill("https://example.test/feed.xml");
    await page.getByRole("button", { name: /Add Source/i }).click();
    await expect(page.locator("#feed-sources-dashboard")).toContainText("Wave QA Source");
  ...
  test("projects page and global shell expose the agreed workflow navigation", async ({ page }) => {
    ...
    for (const label of ["Home", "Keywords", "Curation lab", "Preview", "Editor", "Send", "Projects", "Config"]) {
